  ...
  return (
    <main className="w-full">
      <Container>
        ...
        <Link href={`/game/${thunderGame.id}`}>
          <section className="w-full bg-black rounded-lg">
            <div className="w-full max-h-96 h-96 relative rounded-lg">
              <div className="absolute z-20 bottom-0 p-3 justify-center items-center flex gap-2">
                <p className="font-bold text-xl text-slate-50">{thunderGame.title}</p>
                <BsArrowRightSquare className="size-6 text-slate-50" />
              </div>
              <Image
                src={thunderGame.image_url}
                alt={thunderGame.title}
                priority={true}
                quality={100}
                fill={true}
                className="max-h-96 object-cover rounded-lg opacity-50 hover:opacity-100 transition-all duration-300"
                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 44vw" //Esta config sempre deixa sobrepondo os items, importante passar uma posição a qual ela é relativa
